  ...
  for (const model of candidateModels) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        const isUnavailableOrRateLimited = err?.status === 503 || err?.code === 503 || errMsg.includes("503") || errMsg.includes("UNAVAILABLE") || errMsg.includes("high demand") || err?.status === 429 || errMsg.includes("429");
...
Respond ONLY with valid JSON in this structure:
{
  ...
  "unrecognizedFields": string[]
    ...
    const errMsg = String(err?.message || err || "");
    const isHighDemand = err?.status === 503 || err?.code === 503 || errMsg.includes("503") || errMsg.includes("UNAVAILABLE") || errMsg.includes("high demand") || err?.status === 429 || errMsg.includes("429");
    return res.status(200).json({
      success: false,
      error: isHighDemand ? "HIGH_DEMAND" : "SERVER_ERROR",
      message: isHighDemand ? "Scanning service is currently experiencing temporary high demand. Please try again in a moment." : "Internet connection required for advanced recognition.",
      details: errMsg
    });
  }
});
